...
// --- Get All Articles (builtin + custom) ---
...
function initArticlePage() {
  ...
  // Share buttons
  const shareBtnTop = document.getElementById('btn-share-article');
  const shareBtnBottom = document.getElementById('btn-share-bottom');
  [shareBtnTop, shareBtnBottom].forEach(btn => {
    if (btn) {
      btn.addEventListener('click', () => {
        if (navigator.share && article) {
          navigator.share({
            title: Lang.getArticleField(article, 'title'),
  ...
  if (publishBtn) {
    publishBtn.addEventListener('click', () => {
      ...
      if (publishBtn) {
        ...
        publishBtn.style.backgroundColor = '#9CA3AF';
        setTimeout(() => {
          window.location.href = 'index.html';
        }, 600);
      }
